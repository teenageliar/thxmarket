					...
					<div class="col-md-4 ">
						<form action="" method="post">
							<div class="form-group <?=form_error('nama') ? 'has-error' :null ?>">
								...
								<input type="text" value="<?=$this->input->post('nama') ?? $row->nama  ?>" id="nama" name="nama" class="form-control">
								<span class="help-block"><?=form_error('nama') ?></span>
							</div>
							<div class="form-group <?=form_error('username') ? 'has-error' :null ?>">
								<label>Username *</label>
								<input type="text" value="<?=$this->input->post('username') ?? $row->username  ?>" id="username" name="username" class="form-control">
								<?=form_error('username') ?>
							</div>
							...
							<div class="form-group <?=form_error('passconf') ? 'has-error' :null ?>">
								...
								<input type="password" value="" id="passconf" name="passconf" class="form-control">
								<?=form_error('passconf') ?>
							</div>
